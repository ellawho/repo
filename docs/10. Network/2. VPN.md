...
1. A running server 
2. Networking firewall - ensure SSH/22, UDP/443 is open and whitelisted the client IP
...
* Add a config file for the SSH: `vim /etc/ssh/sshd_config` 
...
* Local machine ssh in will be `ssh -i ~/.ssh/id_rsa ella@192.168.0.1 -p 88` 

* Create ssh/config file: `vim ~/.ssh/config`, add below content
...
![Firefox-SOCKS5](../assets/images/network/sock5-proxy-firefox.png){:width="60%"}